...
-- NOTE(acol): Helper for generate loop
-------------------------------------------------------------------------------
entity FAHelper is
  Port(
    Clk  : in  std_logic;
...
architecture Structural of SystolicMultiplier4Bit is
  ...
  signal Row0Sin  : std_logic_vector(3 downto 0) := "0000";
...
begin
  -- add 
  Lane0: FAHelperLane port map (Clk, A, B(0), Row0Sin, '0', Row0Sout, Row0Cout);
  -- and shift
  Row1Sin(2 downto 0) <= Row0Sout(3 downto 1);
  Row1Sin(3)          <= Row0Cout;

  Lane1: FAHelperLane port map (Clk, A, B(1), Row1Sin, '0', Row1Sout, Row1Cout);

  Row2Sin(2 downto 0) <= Row1Sout(3 downto 1);
  Row2Sin(3)          <= Row1Cout;

  Lane2: FAHelperLane port map (Clk, A, B(2), Row2Sin, '0', Row2Sout, Row2Cout);

  Row3Sin(2 downto 0) <= Row2Sout(3 downto 1);
  Row3Sin(3)          <= Row2Cout;

  Lane3: FAHelperLane port map (Clk, A, B(3), Row3Sin, '0', Row3Sout, Row3Cout);

  Product(0) <= Row0Sout(0);
  Product(1) <= Row1Sout(0);
  Product(2) <= Row2Sout(0);
